...
func TestLoadConfig_MissingFile(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "notlinktree-missing-config-test-")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	missingPath := filepath.Join(tempDir, "nonexistent_config.yaml")
...
func TestValidatePassword(t *testing.T) {
	tests := []struct {
		...
		// Previously blocked common passwords now allowed (but still need letters + numbers)
		{
		...
		{
			name:        "previously blocked common password case insensitive with numbers",
			password:    "PASSWORD123",
			expectError: false,
		},
		{
			name:        "previously blocked sequential numbers with letters",
		...
		},
		{
			...
			password:    "mypass123测试",
			expectError: false,
		},
		{
			name:        "valid password with spaces in middle",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				if err == nil {
					t.Errorf("Expected error but got nil for password: %q", tt.password)
					return
				}
				...
				if err != nil {
					t.Errorf("Expected no error but got: %v for password: %q", err, tt.password)
				}
			}
		})
	}
