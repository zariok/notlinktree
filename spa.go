...
func spaHandler(fs embed.FS, prefix string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Sanitize and clean the path
		origPath := r.URL.Path
		pathPart := strings.TrimPrefix(origPath, "/"+prefix)
		if pathPart == "" || pathPart == "/" {
			pathPart = "/index.html"
		}
		if pathPart == "/admin" {
			pathPart = "/admin/index.html"
		}
		// Clean the path to prevent traversal oddities
		cleanPath := path.Clean("/" + pathPart)
		filePath := prefix + cleanPath
