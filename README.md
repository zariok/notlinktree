...
### Build everything (UI, embed, Go binary)
Use the makefile to:
- Build the UI
...
NotLinkTree automatically builds and publishes container images to GitHub Container Registry on every release. You can use these images directly without building from source.

**Registry Location:** `ghcr.io/zariok/notlinktree`
...
docker run -d \
  --name notlinktree \
  -p 8080:8080 \
  -e NLT_JWT_SECRET="your-super-secret-jwt-key-here" \
  -v notlinktree_data:/config \
  ghcr.io/zariok/notlinktree:latest
```

#### Change password via container
```bash
docker exec -w /config notlinktree notlinktree -setadminpw "admin123"
```

### Building from Source

For building Docker images from source code, see [README-docker-build.md](README-docker-build.md).
