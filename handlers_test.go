...
func TestAPI_AdminPasswordChange_Validation(t *testing.T) {
	server, teardown := setupTestServer(t)
	defer teardown()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			passwordData := map[string]string{"password": tc.password}
			body, _ := json.Marshal(passwordData)
			...
			if res.StatusCode != tc.expectedStatus {
				bodyBytes, _ := io.ReadAll(res.Body)
				t.Fatalf("Expected status %d, got %d. Body: %s", tc.expectedStatus, res.StatusCode, string(bodyBytes))
			}
			...
			if tc.expectedError != "" {
				...
				if errorRes.Error == nil || errorRes.Error.Details != tc.expectedError {
					t.Errorf("Expected error details %q, got %q", tc.expectedError, errorRes.Error.Details)
				}
			} else {
				var successRes APIResponse
				json.NewDecoder(res.Body).Decode(&successRes)
				if !successRes.Success {
					t.Error("Expected success response for valid password")
				}
			}
		})
	}
...
func TestAPI_AdminPasswordChange_PreservesOtherData(t *testing.T) {
	...
	// Change password
	passwordData := map[string]string{"password": "preservetestpass123"}
